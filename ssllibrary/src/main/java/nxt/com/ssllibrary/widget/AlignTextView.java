package nxt.com.ssllibrary.widget;
...
public class AlignTextView extends android.support.v7.widget.AppCompatTextView {
    ...
    public AlignTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        drawableLeft= typedArray.getInt(R.styleable.AlignTextView_darwableleft,0);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(suffixMode==0){
            ...
            int spec= (int) ((maxTextViewWidth-drawableLeft-chars.length*textPx)/(chars.length-1));
            for (int i = 0; i < chars.length; i++) {
                canvas.drawText(String.valueOf(chars[i]), i * (textPx+spec)+35, baseline, paint);
            }
            //情况二:有后缀
        }else{
            //后缀字符
            ...
            int spec= (int) ((maxTextViewWidth-drawableLeft-chars.length*textPx-suffixWidth)/(chars.length-1));
            for (int i = 0; i < chars.length; i++) {
                    canvas.drawText(String.valueOf(chars[i]), i * (textPx+spec)+drawableLeft, baseline, paint);
            }
